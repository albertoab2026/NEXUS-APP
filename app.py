...
def registrar_cierre_manual_dynamo(usuario_id):
    ...
    try:
        ...
        # Aquí ya está corregido el nombre para que coincida exacto abajo
        dynamodb_resource = boto3.resource('dynamodb', region_name='us-east-1')
        tabla_usuarios = dynamodb_resource.Table('NEXUS_USUARIOS')
    ...
    try:
        ...
        for prod in lista_productos:
            ...
            productos_json.append({
                ...
                'nombre': str(prod.get('nombre') or prod.get('nombre_producto') or prod.get('Producto') or prod.get('descripcion') or 'Artículo sin nombre'),
...
if menu == "Ventas":
            ...
            if st.session_state.carrito:
                ...
                if st.button("⚡ Finalizar y Registrar Venta", type="primary", use_container_width=True):
                    ...
                    for item in st.session_state.carrito:
                        ...
                        # Descontamos el stock y actualizamos DynamoDB con tu función oficial
                        ...
                        for prod in productos:
                            if prod.get('producto_id') == p_id or prod.get('id') == p_id:
                                ...
                                # Obtenemos el precio actual del producto para no alterar su precio de venta
                                precio_actual = float(prod.get('precio_venta', prod.get('precio', 0)))
                                
                                # Llamamos a tu función exacta de DynamoDB
                                actualizar_producto(p_id, precio_actual, nuevo_stock)
    ...
    st.metric(
        "📝 Ganancia Real",
        f"S/{ganancia_hoy:.2f}",
        delta=f"{delta_val:.2f} vs periodo comparativo"
    )
    ...
    with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
        ...
        # Eliminar columnas temporales antes de exportar
        columnas_export = [
            c for c in df_filtrado.columns
            if c != "pago_norm_display"
        ]

        df_filtrado[columnas_export].to_excel(
            writer,
            sheet_name="Ventas_Auditoria",
            index=False
        )
    ...
    nombre_archivo = (
        "Reporte_Turno_Actual.xlsx"
        if tipo_filtro.startswith("Turno")
        else f"Reporte_NEXUS_{fecha_busqueda}.xlsx"
    )
