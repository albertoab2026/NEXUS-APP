...
def obtener_datos():
    try:
        ...
        if df.empty: return pd.DataFrame(columns=['NOMBRE DEL PRODUCTO', 'PRECIO COMPRA (S/)', 'PRECIO PÚBLICO (S/)', 'CANTIDAD DISPONIBLE'])
        ...
        df = df.rename(columns={
            ...
            'Precio': 'PRECIO PÚBLICO (S/)',
            'Stock': 'CANTIDAD DISPONIBLE'
        })
        return df[['NOMBRE DEL PRODUCTO', 'PRECIO COMPRA (S/)', 'PRECIO PÚBLICO (S/)', 'CANTIDAD DISPONIBLE']].sort_values('NOMBRE DEL PRODUCTO')
    except: return pd.DataFrame(columns=['NOMBRE DEL PRODUCTO', 'PRECIO COMPRA (S/)', 'PRECIO PÚBLICO (S/)', 'CANTIDAD DISPONIBLE'])
...
with t1:
    if st.session_state.boleta:
        ...
        bus = st.text_input("🔍 Buscar para vender:").upper()
        prod_lista = [p for p in df_inv['NOMBRE DEL PRODUCTO'].tolist() if bus in str(p)]
        c1, c2 = st.columns(2)
        ...
        if p_sel:
            info = df_inv[df_inv['NOMBRE DEL PRODUCTO'] == p_sel].iloc
            st.info(f"💰 Precio: S/ {float(info['PRECIO PÚBLICO (S/)']):.2f} | 📦 Stock: {int(info['CANTIDAD DISPONIBLE'])}")
            if st.button("➕ Añadir al Carrito", use_container_width=True):
                if cant <= info['CANTIDAD DISPONIBLE']:
                    st.session_state.carrito.append({'Producto': p_sel, 'Cantidad': int(cant), 'Precio': float(info['PRECIO PÚBLICO (S/)']), 'Precio_Compra': float(info['PRECIO COMPRA (S/)']), 'Subtotal': round(float(info['PRECIO PÚBLICO (S/)']) * cant, 2)})
                    st.rerun()
            ...
            if st.session_state.confirmar:
                ...
                if cc1.button("✅ SÍ, PROCESAR", use_container_width=True):
                    ...
                    for i, item in enumerate(st.session_state.carrito):
                        ...
                        n_s = int(df_inv[df_inv['NOMBRE DEL PRODUCTO'] == item['Producto']]['CANTIDAD DISPONIBLE'].values) - item['Cantidad']
...
with t2:
    ...
    bus_stock = st.text_input("🔍 Buscar por nombre del producto:", key="bus_stock").upper()
    if not df_inv.empty:
        df_filtrado = df_inv[df_inv['NOMBRE DEL PRODUCTO'].str.contains(bus_stock, na=False)]
        st.dataframe(df_filtrado, use_container_width=True, hide_index=True,
            column_config={
                "NOMBRE DEL PRODUCTO": st.column_config.TextColumn("NOMBRE DEL PRODUCTO", width="large"),
                "PRECIO COMPRA (S/)": st.column_config.NumberColumn("P. COMPRA", format="S/ %.2f"),
                "PRECIO PÚBLICO (S/)": st.column_config.NumberColumn("P. VENTA", format="S/ %.2f"),
                "CANTIDAD DISPONIBLE": st.column_config.NumberColumn("STOCK ACTUAL")
            })
...
with t5:
    with st.form("carga"):
        st.write("📥 **Registrar Producto**")
        p_n = st.text_input("Producto:").upper().strip()
        s_n = st.number_input("Stock:", min_value=0)
        pr_n = st.number_input("Precio Público:", min_value=0.0); pc_n = st.number_input("Precio Costo:", min_value=0.0)
...
with t6:
    if not df_inv.empty:
        p_edit = st.selectbox("Gestionar:", df_inv['NOMBRE DEL PRODUCTO'].tolist())
